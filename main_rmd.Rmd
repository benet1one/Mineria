...
Initially, the cleaned and overampled dataset was divided into a training and a test set, in order to obtain reliable results and assess the model’s ability to generalize to unseen data. The next step involved defining the features and the numerical target variable within these subsets.
...
Finally, the parameter k (number of neighbors) was optimized based on the RMSE results, with the best performance obtained for k = 25, the highest possible value of k.
This analysis indicates that KNN is a computationally expensive method for further exploration, because the KPI gets better as k increases. Therefore, testing alternative models might be advisable for improved efficiency and scalability.
